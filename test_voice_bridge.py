...
def test_claude_cli():
    ...
    try:
        ...
        print("   ❌ Claude Code CLI - 响应超时")
        return False
    except FileNotFoundError:
        print("   ❌ Claude Code CLI - 未找到")
        print("   请安装 Claude Code: https://claude.ai/code")
        return False
        ...
        for i in range(device_count):
            try:
                ...
                if info['maxInputChannels'] > 0:
                    input_devices.append(f"   🎤 {info['name']}")
            except:
                continue
...
def test_whisper_model():
    ...
    try:
        ...
        try:
            ...
            print("   ⚠️ Whisper small 模型加载失败（将使用base模型）")
...
def main():
    ...
    if passed == total:
        print("🎉 所有测试通过！语音桥接器可以正常使用")
    elif passed >= total * 0.8:
        print("⚠️ 大部分测试通过，可以尝试使用")
    else:
        print("❌ 多项测试失败，请检查系统环境")
